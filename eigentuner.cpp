...
int output_setting;	// Has value 1 or 2 depending on whether output files are created on every iteration or only the last one
...
double F_orig;  // Variable for memorizing original value of F if it needs to be changed to get over some particular iteration without missing an eigenvalue
	...
	// The loop for performing the appropriate number of energy tuning iterations
		...
		for(int n=0; n<Enum; n++) {
			for(int m=0; m<=n; m++) {
				...
				intgrl=0.0;
				for(int qq=0; (double)qq*step<0.8*length; qq++) {
					intgrl += 2.0*step*psivals[n][qq]*psivals[n][qq]*psivals[m][qq]*psivals[m][qq];
				}
				linsys[m][n]=intgrl; // The matrix of this linear system is symmetric
				linsys[n][m]=intgrl;
			}
		}
		...
		for(int p = 0; p<Enum; p++) {	// Adjust the potential energy function for the next iteration
			for(int r = 0; (double)r*step<0.8*length; r++) {
				pe[r] += F*rhs[p][0]*psivals[p][r]*psivals[p][r]; // The perturbation terms are proportional to square moduli of previous wave functions
			}
		}
		...
		for(int n=0; n<Enum; n++) { 
			if(abs(Evalues[n]-Eprev[n])>5.0*abs(Eprev[n]-Etarget[n]) && abs(Eprev[n]-Etarget[n]) < abs(Evalues[n] - Etarget[n]) && abs(Evalues[n]-Eprev[n])>4.0*E1lb) repeat=1;
		}
		
		if(repeat==1) cout << "Failed iteration step " << (iter+1) << ", repeating it with different value of F.\n\n";
