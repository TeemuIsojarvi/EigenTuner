// EigenTuner v0.95
//
// This program finds a 1D potential energy function V(x), which has the property V(x)=V(-x) and which results in the first N eigenenergies
...
// well of given depth and width, and the program seeks a perturbation term that shifts the lowest energies to the appropriate values given by the user.
//
// The computation is done with the shooting method and 4th order Runge-Kutta integration.
// Teemu Isojärvi 12.08.2021
...
int idx=0;
int npoints=90000;	// Number of points in the x-discretization
int ni;		// Number of iterations used in the tuning
int Enum; 	// Number of eigenvalues to adjust
int output_setting;	// Produce a V(x) output file on every iteration or only the last?
int partition;	// How many parts is the well divided to when forming a step-like perturbation term?
int r;
...
double *pe_prev = new double[100000];
...
double (*step_fnc)[100000] = new double[30][100000];	// Bar-like perturbation functions, constant on one interval and zero elsewhere
...
double solve_eigenstates() { // A subprogram for solving the N energies and probability densities on each iteration
	...
	norm[m]=0.0;
	for(int count=0; (count<npoints && (double)count*step<0.7*length); count++) norm[m]+=2.0*psivals[m][count]*psivals[m][count]*step;
	for(int count=0; (count<npoints && (double)count*step<0.7*length); count++) psivals[m][count]/=sqrt(norm[m]);
...
// This is the 'main' subprogram for this application
	...
	double middle_points[partition];

	for(int m=0; m<partition; m++) {
	middle_points[m]=(double)m*length/(2.0*(double)partition-1.0);
	}

	for(int m=0; m<partition; m++) {
		for(int n=0; n<npoints/4; n++) {
			x=(double)n*step;
			if(abs(x-middle_points[m])<length/(2.0*(2.0*(double)partition-1.0))) step_fnc[m][n]=1.0*partition/length;
			else step_fnc[m][n]=0.0;
		}
	}
	...
	// The loop for performing the appropriate number of energy tuning iterations
		...
		for(int n=0; n<npoints; n++) pe_prev[n]=pe[n];
		...
		for(int n=0; n<partition/2; n++) {
			for(int m=0; m<partition/2; m++) {
		// These loops form the square matrix for the linear system to be solved on each iteration
				...
				for(int qq=0; (double)qq*step<0.6*length; qq++) {
					// Even and odd-numbered bar functions are alternated on even and odd iterations, to get more subdivisions in V(x)
					// When tuning n eigenvalues, the bottom of the well is divided to 4n-1 parts where each PT differs from 0
					if(iter%2==1) {
						intgrl += 2.0*step*step_fnc[2*n][qq]*psivals[m][qq]*psivals[m][qq];
					}
					else {
						intgrl += 2.0*step*step_fnc[2*n+1][qq]*psivals[m][qq]*psivals[m][qq];
					}
		...
		for(int p = 0; p<Enum; p++) {	// Adjust the potential energy function for the next iteration
			for(r = 0; (double)r*step<0.6*length; r++) {
				if(iter%2==1) {
					pe[r] += F*rhs[p][0]*step_fnc[2*p][r]; // The perturbation terms are histogram-like bars, with tunable height
					
				}
				else {
					pe[r] += F*rhs[p][0]*step_fnc[2*p+1][r];
		...
		for(r=1; (double)r*step<0.6; r++) {
			// Remove artifacts (point discontinuities) from the V(x) function
			if(pe[r-1]-pe[r]>0.0 && pe[r+1]-pe[r]>0.0) pe[r]=pe[r-1];
			if(pe[r-1]-pe[r]<0.0 && pe[r+1]-pe[r]<0.0) pe[r]=pe[r-1];
		}
		...
		for(int n=0; n<Enum; n++) { 
			if(abs(Evalues[n]-Eprev[n])>3.0*abs(Eprev[n]-Etarget[n]) && abs(Eprev[n]-Etarget[n]) < abs(Evalues[n] - Etarget[n]) && abs(Evalues[n]-Eprev[n])>4.0*E1lb) repeat=1;
		}
		
		if(repeat==1) cout << "Failed iteration step " << (iter+1) << ", repeating it with different perturbation terms.\n\n";

		//****************************************************************************************************************

		if(repeat==1) {
			// If the change in energies from last iteration looks like the iteration step failed, do the same step again
			// with even-numbered bars replaced by odd-numbered ones and vice versa
			
			for(int n=0; n<npoints; n++) pe[n]=pe_prev[n];
			for(int n=0; n<Enum; n++) Evalues[n]=Eprev[n];
