...
// well of given depth and width, or a harmonic oscillator of given spring constant, and the program seeks a perturbation term
// that shifts the lowest energies to the appropriate values given by the user.
//
// The computation is done with the shooting method and 4th order Runge-Kutta integration.
// Teemu Isojärvi 16.04.2021
...
double solve_eigenstates() { // A subprogram for solving the N energies and probability densities on each iteration
	...
	for(int m=0; m<Enum; m++) { // Loop over the first N eigenfunctions and energies
		...
		{
			E+=0.1*E1lb; // This is the step length when scanning on the energy axis
	...
	{
		E+=0.0002*E1lb;	// The energy step is much shorter in this second energy scanning loop
	...
	for(x=0.0; (x<2.0*length && flag>0.5); x+=step)
	{
		...
		if(((double)idx-1.0)*step<0.6*length) psivals[m][idx-1]=psi;
	}
	idx=0;
	if(n==1 && ((psiprev>0.0 && psi<0.0) || (psiprev<0.0 && psi>0.0))) { 
		flag=0.0;
